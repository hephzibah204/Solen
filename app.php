...
window.SOLEN_AI_PROVIDER = <?= json_encode(get_setting('ai_provider','claude')) ?>;
</script>
...
function SolenApp() {
  ...
  useEffect(() => {
    (async () => {
      const [pm, mm, sm, md] = await Promise.all([ apiData('get_profile'), apiData('get_memory'), apiData('get_session'), apiData('get_moods') ]);
      setMoods(md.moods || []);
      setMemory(mm.memory || []);
      if (pm?.profile?.coach_name) {
        setProfile(pm.profile); setProgDay(pm.profile.program_day || 0); setPhase("chat");
